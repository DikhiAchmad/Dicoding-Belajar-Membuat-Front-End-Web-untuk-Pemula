...
function templateArtikel(title, author, year, isComplete) {
    ...
    if (isComplete) {
        action.appendChild(belumButton());
        action.appendChild(deleteButton());
        book_item.appendChild(action);
    } else {
        action.appendChild(selesaiButton());
        action.appendChild(deleteButton());
        book_item.appendChild(action);
    }
    return book_item;
...
function removeData(parent) {
    const konfirmasi = confirm("apakah anda ingin menghapus ini ?");
    if (konfirmasi) {
...
function addData() {
    ...
    const datas = templateArtikel(
        inputBookTitle.value,
        "Penulis: " + inputBookAuthor.value,
        "Tahun: " + inputBookYear.value,
        inputBookIsComplete.checked
    );
    const objekData = Objek(
        inputBookTitle.value,
